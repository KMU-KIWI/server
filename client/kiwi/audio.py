...
def audio_db(samples):
    ...
    """
    rms = librosa.feature.rms(y=samples, frame_length=samples.shape[0], center=False)[
        0
    ][0]
...
class AudioWavStream:
    ...
    def next(self):
        ...
        chunk = self.samples[
            self.position : min(self.position + self.chunk_size, len(self.samples))
        ]
...
class AudioMicStream:
    ...
    def open(self):
        ...
        for sample_rate, chunk_size in zip(sample_rates, chunk_sizes):
            try:
                logging.info(
                    f"trying to open audio input {self.device_id} with sample_rate={sample_rate} chunk_size={chunk_size}"
                )
                ...
                print(err)
                logging.warning(
                    f"failed to open audio input {self.device_id} with sample_rate={sample_rate}"
                )
                self.stream = None

        if self.stream is None:
            logging.error(
                f"failed to open audio input device {self.device_id} with any of these sample rates:"
            )
            logging.error(str(sample_rates))
            raise ValueError(
                f"audio input device {self.device_id} couldn't be opened or does not support any of the above sample rates"
            )
    ...
    def next(self):
        ...
        if self.sample_rate != self.device_sample_rate:
            ...
            samples = librosa.resample(
                samples, self.device_sample_rate, self.sample_rate
            )

            if len(samples) != self.chunk_size:
                logging.warning(
                    f"resampled input audio has {len(samples)}, but expected {self.chunk_size} samples"
                )
...
class AudioOutput:
    ...
    def __init__(self, device, sample_rate, chunk_size=4096):
        ...
        if device is None:
            self.device_id = None
            logging.warning("creating pass-through audio output without a device")
            return
    ...
    def open(self):
        ...
        try:
            ...
            logging.error(
                f"failed to open audio output device with sample_rate={self.requested_rate}, trying again with sample_rate={self.sample_rate}: {str(e)}"
            )
        ...
        logging.info(
            f"opened audio output device {self.device_id} ({self.device_info['name']})"
        )
...
def _get_audio_devices(audio_interface=None):
    ...
    for i in range(0, numDevices):
        _audio_device_info.append(
            interface.get_device_info_by_host_api_device_index(0, i)
        )
...
def find_audio_device(device, audio_interface=None):
    ...
    try:
        ...
        if not isinstance(device, str):
            raise ValueError(
                "expected either a string or an int for 'device' parameter"
            )
...
def list_audio_inputs():
    ...
    for i, dev_info in enumerate(devices):
        if (dev_info.get("maxInputChannels")) > 0:
            print(
                "Input Device ID {:d} - '{:s}' (inputs={:.0f}) (sample_rate={:.0f})".format(
                    i,
                    dev_info.get("name"),
...
def list_audio_outputs():
    ...
    for i, dev_info in enumerate(devices):
        if (dev_info.get("maxOutputChannels")) > 0:
            print(
                "Output Device ID {:d} - '{:s}' (outputs={:.0f}) (sample_rate={:.0f})".format(
                    i,
                    dev_info.get("name"),
